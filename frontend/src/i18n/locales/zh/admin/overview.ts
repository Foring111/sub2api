export default {
    ...
    groups: {
      ...
      openaiLive: {
        title: 'OpenAI Live',
        allow: '允许访问 Live',
        hint: '启用后，此 OpenAI 分组的 API Key 可以创建并控制 Live 语音会话。默认关闭。运行 炽热API 的服务端必须是 Apple Silicon Mac，并安装官方 ChatGPT App；客户端平台不受限制。',
        unsupportedTitle: '当前服务端不支持 Live',
        unsupportedMessage: '当前 炽热API 服务端无法生成 Live 所需的设备证明，即使开启也不能使用。是否仍然开启？',
        enableAnyway: '仍然开启'
      },
      openaiFast: {
