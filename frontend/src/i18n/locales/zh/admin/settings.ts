export default {
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 炽热API 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 炽热API 用户登录',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，炽热API 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      upstreamBillingProbe: {
        title: '上游倍率自动探测',
        description: '定期获取 API Key 账号所连接上游 炽热API 站点声明的计费倍率；只有另行开启“同步上游声明倍率”的账号才会更新账号倍率。',
        enabled: '启用全局自动探测',
      ...
      payment: {
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 炽热API 订单中，上游 type 会作为易支付 type 参数提交。',
      ...
      openaiFastPolicy: {
        ...
        scopeBedrock: '仅 Bedrock 账号',
        userIds: '指定用户',
        userIdsHint: '输入任意邮箱关键词进行模糊搜索。留空表示对全部 炽热API 用户生效；选中用户的 API Key 请求优先匹配用户规则。',
