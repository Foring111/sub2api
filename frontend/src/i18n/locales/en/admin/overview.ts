export default {
    ...
    groups: {
      ...
      openaiLive: {
        ...
        hint: 'When enabled, API keys in this OpenAI group can create and control Live voice sessions. Disabled by default. The ChiRe API server must run on Apple Silicon macOS with the official ChatGPT app installed; client platforms are unrestricted.',
        unsupportedTitle: 'Current server does not support Live',
        unsupportedMessage: 'This ChiRe API server cannot generate the required Live attestation. Live will not work even if enabled. Continue anyway?',
